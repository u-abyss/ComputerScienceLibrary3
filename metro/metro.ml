...
(* 目的：ローマ字の駅名（文字列）と駅名リスト（ekimei_t list 型）を受け取ったら、その駅の漢字表記を文字列で返す関数 *)
(* romaji_to_kanji : string -> ekimei_t list -> string *)
let rec romaji_to_kanji ekimei ekimei_list =
  match ekimei_list with
  | [] -> ""
  | { kanji = kj; kana = kn; romaji = r; shozoku = s } :: rest ->
      if r = ekimei then kj else romaji_to_kanji ekimei rest

(* tests *)
let test1 = romaji_to_kanji "myogadani" global_ekimei_list = "茗荷谷"
let test2 = romaji_to_kanji "sapporo" global_ekimei_list = ""

(* 漢字の駅名をふたつ（いずれも文字列）と駅間リスト（ekikan_t list 型）を受け取ったら、
    駅間リストの中からその2駅間の距離を返す関数*)
(* get_ekikan_kyori : string -> string -> ekikan_t list -> float *)

let rec get_ekikan_kyori eki1 eki2 lst =
  match lst with
  | [] -> infinity
  | { kiten = k; shuten = s; keiyu = ku; kyori = kyo; jikan = j } :: rest ->
      if (k = eki1 && s = eki2) || (k = eki2 && s = eki1) then kyo
...
(* 目的：ローマ字の駅名をふたつ受け取ってきたら、その間の距離を調べ、
    - 直接つがっている場合は「A駅からB駅までは ◯ km です」という文字列を返し
    - つながっていない場合は「A駅とB駅はつながっていません」という文字列を返す
    入力されたローマ字の駅名が存在しなかった場合には「~という駅は存在しません」という文字列を返す
*)
(* kyori_wo_hyoji : string -> string -> string *)
let kyori_wo_hyoji eki1 eki2 =
  let kanji1 = romaji_to_kanji eki1 global_ekimei_list
  and kanji2 = romaji_to_kanji eki2 global_ekimei_list in
  if kanji1 = "" then eki1 ^ "という駅は存在しません"
  else if kanji2 = "" then eki2 ^ "という駅は存在しません"
  else
...
(* make_eki_list : ekimei_t -> eki_t list *)
let rec make_eki_list lst =
  match lst with
  | [] -> []
  | { kanji = kj; kana = kn; romaji = rj; shozoku = sho } :: rest ->
...
(* 目的：eki_t 型のリストと起点（漢字の文字列）を受け取ったら、起点のみ以下のようになっている eki_t 型のリストを返す
    - saitan_kyori = 0
...
let rec shokika eki_lst kiten =
  ...
  | ({ namae = n; saitan_kyori = s; temae_list = tl } as first) :: rest ->
      if n = kiten then
        { namae = n; saitan_kyori = 0.; temae_list = [ kiten ] }
        :: shokika rest kiten
...
(* 目的：直前に確定した駅 p (eki_t 型)と未確定の駅 q (eki_t 型)を受け取り、
    p と q が直接つながっているかどうかを調べる。
    - つながっていたら q の最短距離と手前リストを必要に応じて更新したものを返す
...
(* 直前に確定した駅 p （eki_t 型）と未確定の駅のリスト v （eki_t list 型）を受け取り、
    必要な更新処理を行ったあと未確定の駅のリストを返す関数
*)
(* koushin : eki_t -> eki_t list -> eki_t list *)
(* let rec koushin p lst =
    match lst with
    | [] -> []
    | first :: rest -> koushin1 p first :: koushin p rest *)
